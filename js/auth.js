...
async function requestSignupOtp(email) {
    try {
        const response = await fetch(`${BASE_URL}/auth/register/otp/`, {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                'ngrok-skip-browser-warning': 'true'
...
async function submitResult(resultData) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            let todayMinutes = parseInt(localStorage.getItem('todayMinutes')) || 0;
...
function displayStars(containerId, count) {
    ...
    for (let i = 0; i < 5; i++) {
        if (i < count) {
            html += '<span style="color: #fbbf24; font-size: 2.5rem; margin: 0 4px;">⭐</span>';
        } else {
            html += '<span style="color: #374151; font-size: 2.5rem; margin: 0 4px;">☆</span>';
        }
    }
...
function isAuthenticated() {
    return localStorage.getItem('dynacuity_token') !== null;
}
...
function getCurrentUser() {
    const userData = localStorage.getItem('dynacuity_user');
...
/**
 * Render a results table into a container (Matching Android App Look)
 */
function renderResultsTable(results, containerId, limit = 5) {
    const container = document.getElementById(containerId);
    if (!container) return;

    if (!results || results.length === 0) {
        container.innerHTML = '<p style="color: #666; font-size: 0.8rem; margin: 1rem 0;">No sessions recorded yet.</p>';
        return;
    }
    ...
    let html = `
        <div style="margin-top: 1.5rem; text-align: left; background: transparent; border-radius: 0; padding: 0;">
            <div style="font-size: 1.1rem; font-weight: 800; color: #fff; margin-bottom: 1.5rem; font-family: 'Outfit';">Performance Dataset (db.sqlite)</div>
    ...
    displayResults.forEach((r, index) => {
        ...
        const date = dateObj.toLocaleDateString([], { month: '2-digit', day: '2-digit' }) + ' ' + 
                     dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
        
        const accuracy = r.accuracy !== null && r.accuracy !== undefined ? Math.round(r.accuracy) + '%' : '--';
        const avgResp = r.avg_time !== null && r.avg_time !== undefined ? (typeof r.avg_time === 'number' ? r.avg_time.toFixed(0) + 'ms' : r.avg_time) : '--';
        const hits = r.correct_count !== null && r.correct_count !== undefined ? r.correct_count : '--';

        // Check if this is a high score (simulated)
        const isBest = r.score > 20 || (r.game_name === 'Shape Match' && r.score > 800);
        const scoreColor = isBest ? '#22c55e' : '#fff';

        html += `
            <div style="display: flex; align-items: center; padding: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.02); min-height: 40px;">
                <!-- Column 1: Score -->
                <div style="flex: 1; font-size: 0.85rem; font-weight: 800; color: ${scoreColor};">${r.score}</div>
                
                <!-- Column 2: Game / Level -->
                <div style="flex: 3; padding-left: 4px;">
                    <div style="color: #eee; font-weight: 600; font-size: 0.75rem;">${r.game_name}</div>
                    <div style="font-size: 0.65rem; color: #666;">${r.level_info || 'Standard'}</div>
                </div>

                <!-- Column 3: Accuracy -->
                <div style="flex: 1; text-align: center; color: #8B5CF6; font-weight: 800; font-size: 0.75rem;">${accuracy}</div>
                
                <!-- Column 4: Speed -->
                <div style="flex: 1; text-align: center; color: #666; font-size: 0.7rem;">${avgResp}</div>

                <!-- Column 5: Date -->
                <div style="flex: 2; text-align: right; color: #666; font-size: 0.65rem; white-space: nowrap;">${date}</div>

                <!-- Column 4: Index -->
                <div style="width: 32px; text-align: center; color: #333; font-size: 0.7rem;">${index + 1}</div>
            </div>
        `;
    });
